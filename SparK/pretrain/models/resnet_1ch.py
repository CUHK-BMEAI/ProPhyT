...
def create_resnet_1ch(base_model_name='resnet50', in_chans=1, **kwargs):
    """
    创建支持单通道输入的ResNet模型（用于NCCT）
    
    Args:
        base_model_name: 基础ResNet模型名称 (resnet50, resnet101, etc.)
        in_chans: 输入通道数，默认1（NCCT灰度图）
    ...
    with torch.no_grad():
        if old_conv1.weight.shape[1] == 3:
            # 使用3个通道权重的均值
            new_conv1.weight[:, 0:1, :, :].copy_(old_conv1.weight.mean(dim=1, keepdim=True))
        else:
...
@register_model
def resnet50_1ch(pretrained=False, **kwargs):
    """ResNet50 with 1-channel input for NCCT grayscale images"""
    return create_resnet_1ch('resnet50', in_chans=1, **kwargs)


@register_model
def resnet101_1ch(pretrained=False, **kwargs):
    """ResNet101 with 1-channel input for NCCT grayscale images"""
    return create_resnet_1ch('resnet101', in_chans=1, **kwargs)


@torch.no_grad()
def convnet_test():
    from timm.models import create_model
    cnn = create_model('resnet50_1ch')
    ...
    B, C, H, W = 4, 1, 224, 224  # 1 channel for NCCT

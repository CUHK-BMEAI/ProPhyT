...
def get_boxes_from_mask(mask, box_num=1, std = 0.1, max_pixel = 5):
    ...
    # If mask is empty (all black), return a default box covering the entire image
    if len(boxes) == 0:
        h, w = mask.shape
        # Return a box covering the entire image (with some margin)
        boxes = [(0, 0, h, w)] * box_num
...
def train_transforms(img_size, ori_h, ori_w):
    ...
    if ori_h < img_size and ori_w < img_size:
        t.append(A.PadIfNeeded(min_height=img_size, min_width=img_size, border_mode=cv2.BORDER_CONSTANT, value=(0, 0, 0)))
    else:
        t.append(A.Resize(img_size, img_size, interpolation=cv2.INTER_NEAREST))

    t.extend([
        A.HorizontalFlip(p=0.5),
        A.VerticalFlip(p=0.2),
        A.RandomRotate90(p=0.3),
        # 新版Albumentations中ShiftScaleRotate不再接受value/mask_value参数
        A.ShiftScaleRotate(shift_limit=0.05, scale_limit=0.1, rotate_limit=15, border_mode=cv2.BORDER_CONSTANT, p=0.5),
        # 确保后续颜色增强使用float32，避免OpenCV对float64报错
        A.Lambda(image=lambda x, **kwargs: x.astype(np.float32)),
        # 部分环境下GaussNoise参数不兼容，改用更兼容的组合
        A.OneOf([
        ...
        # 将标准化放到增强流水线，避免ColorJitter/RandomGamma对标准化后的负值操作导致CV_64F异常
        A.Normalize(mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], max_pixel_value=255.0),
        ToTensorV2(p=1.0),
    ])
    return A.Compose(t, p=1.0)


# def test_transforms(img_size, ori_h, ori_w):
#     """
#     测试时的确定性预处理，不包含任何随机增强
#     只做resize/pad、标准化和转tensor，确保测试结果可复现
#     """
#     transforms = []
#     if ori_h < img_size and ori_w < img_size:
